...
class obj_sel(ntuplize):
    ...
    def run(self):
        ...
        if not self.data:
            ...
            npos = len(events.Generator.weight)
            nneg = 0
            ntot = len(events.Generator.weight)
            # lumi mask
            lumi_mask = fc.get_lumi_mask(events, self.year)
        ...
        good_muons['pt'] , good_muons['pt_roccor_up'] , good_muons['pt_roccor_down'] = fc.apply_rochester_correction(good_muons,self.data,self.year)
        if not self.data:
        ...
        if not self.data:
            ...
            corrected_jets = fc.apply_jet_corrections('2018').build(good_jets, lazy_cache=events_cache)
        ...
        if not self.data:
            ...
            good_jets['btagSF'], good_jets['btagSF_up'], good_jets['btagSF_down']  = fc.get_btagsf(flav, abseta, pt, '2018')
        ...
        stime = fc.cout("start", "MET selection")
        MET = events.MET
        MET['pt_orig'] = MET.pt
        MET['phi_orig'] = MET.phi
        MET['pt_roccor'], MET['phi_roccor'] = fc.corrected_polar_met(MET.pt,MET.phi,good_muons.pt,good_muons.phi,good_muons.pt_orig)
        ...
        if not self.data:
            ...
            MET['pt'], MET['phi'] = fc.corrected_polar_met(MET['pt_roccor'],MET['phi_roccor'],good_jets["pt"],good_jets["phi"],good_jets["pt_orig"])
            # uncertainties
            MET['pt_roccor_up'], MET['phi_roccor_up'] = fc.corrected_polar_met(MET.pt,MET.phi,good_muons.pt_roccor_up,good_muons.phi,good_muons.pt)
            MET['pt_roccor_down'], MET['phi_roccor_down'] = fc.corrected_polar_met(MET.pt,MET.phi,good_muons.pt_roccor_down,good_muons.phi,good_muons.pt)
            MET['pt_UnclusteredEnergy_up'], MET['phi_UnclusteredEnergy_up'] = fc.corrected_polar_met(
                MET['pt'],
                MET['phi'],
                good_jets["pt"],
                good_jets["phi"],
            ...
            MET['pt_UnclusteredEnergy_down'], MET['phi_UnclusteredEnergy_down'] = fc.corrected_polar_met(
                MET['pt'],
                MET['phi'],
                good_jets["pt"],
                good_jets["phi"],
            ...
            for ibr in jesr_unc:
                MET[f"pt_{ibr}_up"], MET[f"phi_{ibr}_up"] = fc.corrected_polar_met(MET['pt'],MET['phi'],good_jets[f"pt_{ibr}_up"],good_jets["phi"],good_jets["pt"])
                MET[f"pt_{ibr}_down"], MET[f"phi_{ibr}_down"] = fc.corrected_polar_met(MET['pt'],MET['phi'],good_jets[f"pt_{ibr}_down"],good_jets["phi"],good_jets["pt"])
        fc.cout("end","MET selection",entries=ak.sum(events.run!=None),start_time=stime)
        ...
        trig_dict['metFilter'] = fc.get_metFilter(events,self.year)

        # pu weights
        if not self.data:
            pu_wgt, pu_wgt_up, pu_wgt_down = fc.get_pusf(events.Pileup.nTrueInt, self.year)
...
if __name__ == '__main__':
    if args.step == "obj_sel":
        the_step = obj_sel(args.input, args.output, args.year, args.data)
        the_step.run()     
    else:
        fc.cout("Please choose a valid step")
        exit(0)